...
(define-option log-level
  #:default "info"
   (let ([lvl (string->symbol log-level)])
    (if (member lvl '(none fatal error warning info debug))
        lvl
        'info)))
...
(define-option session-secret-key-path
  #:default (build-path (find-system-path 'temp-dir) "shorty-secret-key"))
